...
strengths:
  ✦ Dev background → understands attack surfaces from inside
  ✦ Evidence-based analytical thinking
  ✦ End-to-end ML pipeline experience on AWS
  ✦ Clear technical writing — writeups & documentation
...
### ⚔️ BreachCraft
> *Romper para entender. Construir para asegurar.*

Offensive security lab — documented attack chains on controlled CTF environments.

```
Stack: Python · Bash · Kali Linux
Env:   TryHackMe · HackTheBox
```
...
- EDA → normalization → train/test split → evaluation
- Logistic Regression with vectorized gradient descent + L2 reg
...
### 🎵 JamRoom
> *Real-time synchronized music, shared live*

WebSocket-based platform for multi-user synchronized audio sessions.
...
- Real-time bidirectional sync with WebSocket channels
- Session auth and room access control
...
### 🏛️ Colliseum
> *Microservice architecture for institutional resource management*

Production-grade backend with RBAC, API Gateway, and CI/CD on Azure.

```
Stack: Java 17 · Spring Boot · MongoDB Atlas
Cloud: Azure · GitHub Actions · API Gateway
```
...
> *Document intelligence with vector search & LLM grounding*

Full RAG pipeline from raw documents to queryable, grounded LLM responses.
...
**Busco prácticas profesionales en ciberseguridad, desarrollo de software o ML/data engineering.**
**Disponible de inmediato.**

<br>

*Open to internships in cybersecurity, software development, or ML/data engineering — available immediately.*
...
```bash
$ echo "Good music + a stubborn bug + finding a flag after 3h of enum = perfection 🎵💻🔐"
```
